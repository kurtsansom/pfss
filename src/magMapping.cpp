...
bool MagMapping::exportImageExpansionFactor()
{
	...
	for(uint j=0; j<numTheta; ++j)
		for(uint k=0; k<numPhi; ++k)
		{
			...
			//*
			Vec3D pos_l	= magl.posdata[0];
			Vec3D pos_u	= magl.posdata[magl.numSamples-1];

			Vec3D pos_lc	= pos_l.convCoordSpher2Cart();
			Vec3D pos_uc	= pos_u.convCoordSpher2Cart();

			Vec3D B_l		= magl.magdata[0].convVecSpher2Cart(pos_lc);
			Vec3D B_u		= magl.magdata[magl.numSamples-1].convVecSpher2Cart(pos_uc);

			hcFloat rl		= pos_l[0];
			hcFloat ru		= pos_u[0];
			hcFloat bl		= B_l.length();
			hcFloat bu		= B_u.length();
	...
	expansion.writeKeyString("BUNIT", "unit of pixel values", "1");
	return retval;
}

bool MagMapping::exportImageExpansionFactorMax()
{
	string fn = getFilename_magMappingExpansionMax(info, height, sinLatFormat, compCoords, numTheta, numPhi);
	if(!checkComputed())		return false;
	if(!checkExportedFile(fn))	return false;
	...
	for(uint j=0; j<numTheta; ++j)
		for(uint k=0; k<numPhi; ++k)
		{
			...
			for(uint l=0; l<magl.numSamples-1; ++l)
				for(uint m=l; m<magl.numSamples; ++m)
				{
					//*
					Vec3D pos_l	= magl.posdata[l];
					Vec3D pos_u	= magl.posdata[m];

					Vec3D pos_lc	= pos_l.convCoordSpher2Cart();
					Vec3D pos_uc	= pos_u.convCoordSpher2Cart();

					Vec3D B_l		= magl.magdata[l].convVecSpher2Cart(pos_lc);
					Vec3D B_u		= magl.magdata[m].convVecSpher2Cart(pos_uc);
					...
					hcFloat factor 	= bl * rl * rl / (bu * ru * ru);
					if (factor > expMax) expMax = factor;
				}

			expansion(k, numTheta - j - 1)	= expMax;
		}
	bool retval 	= true;
